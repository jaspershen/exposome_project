...
setwd("3_data_analysis/data_20200511/exposomeBiological_gutmicrobiome/")
...
exposomeBiological_expression_data1 <- exposomeBiological_expression_data
save(exposomeBiological_expression_data1, file = "exposomeBiological_expression_data1")
save(gutmicrobiome_expression_data1, file = "gutmicrobiome_expression_data1")

####calculate correlation between gutmicrobiome and exposome
cor_value <-
...
p_value <-
  purrr::map(
    ...
    .f = function(x) {
      ...
      value2 <- as.numeric(gutmicrobiome_expression_data1[x[2],])
...
setwd(r4projects::get_project_wd())
setwd(
  "3_data_analysis/data_20200511/exposomeBiological_gutmicrobiome/exposomeBiological_gutmicrobiome_plot"
)
for (idx in 1:20) {
  cat(idx, " ")
  ...
  temp_data <-
    data.frame(
      ...
      exposomeBiological = as.numeric(exposomeBiological_expression_data[cor_value1$from[idx], ]),
      gutmicrobiome = as.numeric(gutmicrobiome_expression_data1[cor_value1$to[idx], ]),
      stringsAsFactors = FALSE
    )
  plot <-
    temp_data %>%
    ggplot(aes(exposomeBiological, gutmicrobiome)) +
    geom_point() +
    geom_smooth(method = "lm", color = "skyblue") +
    ggrepel::geom_label_repel(aes(x = exposomeBiological, gutmicrobiome, label = date)) +
    labs(
...
###correlation network for exposome biological and gutmicrobiome
...
###network for all the exposome biological and gutmicrobiome
edge_data <-
  cor_value1 %>%
...
plot1 <-
  ...
  geom_edge_arc(aes(color = Correlation
                ...
                ggsave(
                  plot1,
                  filename = "exposomeBiological_gutmicrobiome_correlation_network.pdf",
                  width = 8.5,
                  height = 7,
                  bg = "transparent"
                )
                ...
                ggsave(
                  plot1,
                  filename = "exposomeBiological_gutmicrobiome_correlation_network.png",
                  width = 8.5,
                  height = 7,
                  bg = "transparent"
                )
