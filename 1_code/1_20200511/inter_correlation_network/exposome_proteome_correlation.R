...
setwd(r4projects::get_project_wd())
setwd("3_data_analysis/data_20200511/exposome_proteome_correlation_network/")
...
# 
# coords$x[coords$Class == "Proteome"] <-
#   seq(0, 332, length.out = sum(coords$Class == "Proteome"))
# 
# coords$x[coords$Class != "Proteome"] <-
#   seq(0, 332, length.out = sum(coords$Class != "Proteome"))
...
plot <-
  ...
  geom_node_text(
    aes(
      ...
      hjust = ifelse(Class == "Proteome", "inward",'outward'),
      angle = -((-node_angle(x, y) + 90) %% 180) + 90,
      size = ifelse(Class == "Proteome", 2, 4),
      # size = 1,
      colour = Class
    ),
    # size = 3,
...
node_data1 = 
  ...
  dplyr::filter((Class != "Proteome") | (
    Class == "Proteome" & Degree > 5
  ))

edge_data1 = 
  edge_data1 %>%
  dplyr::filter((from %in% node_data1$node[node_data1$Class == "Proteome"]) |
                  (to %in% node_data1$node[node_data1$Class == "Proteome"]))
...
plot <-
  ...
  geom_node_text(
    aes(
      x = x * 1.03,
      y = ifelse(Class == "Proteome", y * 0.95, y * 1.03),
      label = ifelse(Class == "Proteome", true_name, true_name),
      hjust = ifelse(Class == "Proteome", "inward",'outward'),
      angle = -((-node_angle(x, y) + 90) %% 180) + 90,
      size = 10,
      colour = Class
