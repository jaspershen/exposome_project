...
#####exposomeChemical
load("3_data_analysis/data_20200511/exposomeChemical_blood_test/cor_value")
...
subnetwork_info =
  readr::read_csv("subnetwork_info.csv")
...
plot <-
  ...
  ggrepel::geom_text_repel(aes(node, edge,
                               label = ifelse(node > 50, subnetwork, NA)))+
  ggforce::facet_zoom(
    xlim = c(0, 70),
    ylim = c(0, 80),
    horizontal = FALSE,
