...
setwd(r4projects::get_project_wd())
load("3_data_analysis/data_20200511/gut_microbiome/expression_data")
gutmicrobiome_expression_data <- expression_data
load("3_data_analysis/data_20200511/gut_microbiome/sample_info")
...
diff <- 
  lapply(gutmicrobiome_sample_info$CollectionDate %>% as.Date(), function(x){
    x <- abs(x - as.Date(environment_sample_info$date.start))
    as.Date(environment_sample_info$date.start) [which(x >=0 & x <= 7)]
  }) 

names(diff) <- gutmicrobiome_sample_info$CollectionDate %>% as.character()
...
diff <- 
  purrr::map2(diff, names(diff), .f = function(x,y){
    ...
    colnames(z) <- c("gutmicrobiome", "environment")
    z
  }) 
...
plot <- 
  ...
  dplyr::mutate(class = factor(class, levels = c("gutmicrobiome", "environment"))) %>% 
  ...
  # scale_y_continuous(breaks = c("gutmicrobiome", "environment"), labels = c("gutmicrobiome", "Exp")) +
  scale_y_discrete(breaks = c("gutmicrobiome", "environment"), labels = c("Proteome", "Exposome")) +
  labs(x = "", y = "") +
...
ggsave(plot, filename = "environment_gutmicrobiome_match.pdf", width = 10, height = 7)
ggsave(plot, filename = "environment_gutmicrobiome_match2.pdf", width = 14, height = 7)
