...
setwd("3_data_analysis/data_20200511/environment_blood_test/")

load("blood_test_expression_data")
load("environment_expression_data")
load("blood_test_sample_info")
load("environment_sample_info")

load("environment_variable_info")

blood_test_sample_info$sample_id == colnames(blood_test_expression_data)
colnames(blood_test_expression_data) <- as.character(blood_test_sample_info$date.start)
colnames(environment_expression_data) <- colnames(blood_test_expression_data)

#######correlation analysis
blood_test_expression_data <- log(blood_test_expression_data + 1, 2)

dim(environment_expression_data)
dim(blood_test_expression_data)

###correct fiber for metabolomics
blood_test_expression_data1 <- 
...
temp_data <- 
  apply(blood_test_expression_data1, 1, function(x){
    (x - mean(x))/sd(x)
  }) %>% 
  t() 

colnames(temp_data) <- 
colnames(blood_test_expression_data1) <-
  colnames(blood_test_expression_data)

##heatmap of blood_test
library(circlize)
...
ggsave(plot, filename = "date.start/blood_test_heatmap.pdf", width = 7, height = 7)
...
environment_expression_data <- 
  environment_expression_data[remain_idx,]

environment_variable_info <- 
  environment_variable_info[remain_idx,]
...
# 
# ####calculate correlation between blood_test and exposome
# cor_value <-
#   cor(x = t(as.matrix(environment_expression_data)),
#       y = t(as.matrix(blood_test_expression_data1)),
#       method = "spearman")
# 
# cor_value <-
#   cor_value %>%
...
#       value2 <- as.numeric(blood_test_expression_data1[x[2], ])
...
setwd(r4projects::get_project_wd())
setwd("3_data_analysis/data_20200511/environment_blood_test/environment_blood_test_plot")
# for(idx in 1:nrow(cor_value1)){
#   cat(idx, " ")
#   path1 <- file.path(cor_value1$from[idx])
#   dir.create(path1, showWarnings = FALSE)
#   temp_data <-
#     data.frame(date = as.character(blood_test_sample_info$date.start),
#                exp = as.numeric(environment_expression_data[cor_value1$from[idx],]),
#                met = as.numeric(blood_test_expression_data1[cor_value1$to[idx],]),
#                stringsAsFactors = FALSE)
#   plot <-
#   temp_data %>%
#     ggplot(aes(exp, met)) +
...
#     labs(x = paste("Exposome (Biological): ", cor_value1$from[idx], sep = ""),
#          y = paste("blood_test: " ,cor_value1$to[idx]), sep = "") +
#     theme_bw() +
...
###correlation network for exposome biological and blood_test
...
###network for all the exposome biological and blood_test
edge_data <-  
  cor_value1 %>% 
...
node_data <- 
  ...
  dplyr::mutate(class1 = case_when(
    stringr::str_detect(class, "from") ~ "Environment",
    TRUE ~ "Blood test"
  )) %>% 
...
# ggsave(
#   plot1,
#   filename = "environment_blood_test_correlation_network.pdf",
#   width = 8.5,
#   height = 7,
#   bg = "transparent"
# )
...
# ggsave(
#   plot1,
#   filename = "environment_blood_test_correlation_network.png",
#   width = 8.5,
#   height = 7,
#   bg = "transparent"
# )
