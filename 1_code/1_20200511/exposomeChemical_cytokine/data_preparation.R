...
setwd("3_data_analysis/data_20200511/exposomeChemical_cytokine/")
...
temp %>% 
  ...
  dplyr::mutate(class = factor(class, levels = c("cytokine", "exp"))) %>% 
  ...
  # scale_y_continuous(breaks = c("cytokine", "exp"), labels = c("cytokine", "Exp")) +
  scale_y_discrete(breaks = c("cytokine", "exp"), labels = c("Cytokine", "Exposome")) +
  labs(x = "", y = "") +
...
ggsave(plot, filename = "exp_cytokine_match.pdf", width = 10, height = 7)
ggsave(plot, filename = "exp_cytokine_match2.pdf", width = 14, height = 7)
