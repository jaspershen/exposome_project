...
setwd("3_data_analysis/data_20200511/environment_cytokine/")
...
colnames(environment_expression_data) <- colnames(cytokine_expression_data)
...
# ###calculate correlation between cytokine and environment
# cor_value <-
...
cor_value1 <-
  cor_value1 %>%
  dplyr::mutate(environment_id = from) %>%
  dplyr::mutate(cytokine_id = to) %>%
  dplyr::filter(!is.na(cytokine_id))

setwd(r4projects::get_project_wd())
setwd("3_data_analysis/data_20200511/environment_cytokine/environment_cytokine_plot")
...
###network for all the environment and cytokine
edge_data <-
  cor_value1 %>%
...
ggsave(
  plot1,
  filename = "environment_cytokine_correlation_network.pdf",
  width = 8.5,
  height = 7,
  bg = "transparent"
)
...
ggsave(
  plot1,
  filename = "environment_cytokine_correlation_network.png",
  width = 8.5,
  height = 7,
  bg = "transparent"
)
