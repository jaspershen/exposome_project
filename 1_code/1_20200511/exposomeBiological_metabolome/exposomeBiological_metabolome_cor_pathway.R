...
source("R/20200511/exposomeChemical_metabolome/pathway_enrichment.R")
...
met_annotation_info <-
  readr::read_csv("3_data_analysis/data_20200511/metabolome/annotation_table.csv")

setwd("3_data_analysis/data_20200511/exposomeBiological_metabolome/")
...
marker_info <- 
  cor_value1 %>% 
  # dplyr::filter(microbiome_name %in% cluster2) %>%
  dplyr::select(to) %>% 
  dplyr::distinct(to) %>% 
...
plot = 
  ...
  ggrepel::geom_label_repel(aes(x = score, y = 0, label = module),
                            data = data.frame(module = paste("Module", 1:length(activity.score)),
                                              score = activity.score)[c(3,6,10),,drop = FALSE]) +
  geom_vline(xintercept = activity.score[c(3,6,10)], color = "red") +
...
#                colNames = TRUE, rowNames = FALSE)
# saveWorkbook(wb, "Exposome_biological_metabolome_enriched_pahtway.xlsx", overwrite = TRUE)
  ...
  purrr::map(function(x){
    ...
    x <- 
      ...
      dplyr::left_join(cor_value1[,c(1,2,3)] %>% 
                         dplyr::rename(microbe_id = from, 
                                       metabolite_id = to, 
                                       correlation = cor), 
                       by = c("from" = "metabolite_id"))
    colnames(x) <- c("metabolite_id", "microbe_id", "correlation")
    x
  })
  ...
  purrr::map2(
    ...
    .f = function(x, y) {
      data.frame(x, pathway = y) %>%
        dplyr::select(microbe_id, metabolite_id, pathway, correlation)
    }
  )
...
metabolite_id <-
  ...
  dplyr::left_join(microbiome_variable_info[,c(1,2)],
                   by = c("microbe_id" = "variable_id")) %>% 
  dplyr::select(short_name, everything()) %>% 
  dplyr::select(-microbe_id) %>% 
  dplyr::rename(microbe_id = short_name)
...
###only remain the top 20 microbes
metabolite_id_data2 <-
  metabolite_id_data

top20_name <-
  table(metabolite_id_data2$microbe_id) %>% 
...
metabolite_id_data2 <-
  metabolite_id_data2 %>%
  dplyr::filter(
    microbe_id %in% top20_name
  )

temp_data1 <- 
  metabolite_id_data2 %>%
  dplyr::select(pathway, microbe_id, correlation) %>%
  dplyr::mutate(affect = case_when(correlation > 0 ~ "pos",
                                   correlation < 0 ~ "neg")) %>%
  dplyr::select(-c(correlation)) %>%
  dplyr::distinct() %>%
  dplyr::select(microbe_id, affect) %>%
  dplyr::mutate(microbe_id = factor(microbe_id, levels = names(sort(
    table(microbe_id),
    decreasing =
      FALSE
  )))) 
...
# ggsave(plot1, filename = "plot1.pdf", width = 4.5, height = 7)
...
edge_data <-
  rbind(
    metabolite_id_data2[, c("microbe_id", "metabolite_id", "correlation")] %>%
      dplyr::distinct(microbe_id, metabolite_id, .keep_all = TRUE) %>%
      dplyr::rename(from = microbe_id,
                    to = metabolite_id,
                    cor = correlation),
...
node_data <-
  rbind(
    metabolite_id_data2[, "microbe_id", drop = FALSE] %>%
      dplyr::distinct() %>%
      dplyr::mutate(class = "microbe") %>%
      dplyr::rename(node = microbe_id),
...
node_data[node_data$class == "microbe", ] <-
  node_data[node_data$class == "microbe", ][match(levels(temp_data1$microbe_id),
...
# 
# coords$x[coords$class == "metabolite"] <- 
#   seq(0, 332, length.out = sum(coords$class == "metabolite"))
# 
...
my_layout1$y[my_layout1$class == "microbe"] <-
  seq(from = 1, to = 100, length.out = sum(my_layout1$class == "microbe"))
...
# ggsave(plot,
#        filename = "exposome_biological_metabolite_pathway.pdf",
#        width = 9,
#        height = 7)
...
ggsave(plot1, filename = "plot1_all.pdf", width = 4.5, height = 14)
...
# ggsave(plot2, filename = "plot2_all.pdf", width = 7, height = 14)
...
# ggsave(plot,
#        filename = "exposome_biological_metabolite_pathway_all.pdf",
#        width = 9,
#        height = 14)
