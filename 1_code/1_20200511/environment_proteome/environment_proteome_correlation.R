...
setwd("3_data_analysis/data_20200511/environment_proteome/")
...
ggsave(plot,
       filename = "proteome_plot/exposome_heatmap.pdf",
       width = 7,
       height = 7)
...
# cor_value <-
#   cor(x = t(as.matrix(proteome_expression_data1)),
...
#     .f = function(x) {
#       value1 <- as.numeric(proteome_expression_data[x[1],])
...
setwd(r4projects::get_project_wd())
setwd("3_data_analysis/data_20200511/proteome_environment/proteome_environment_plot")
...
#       exp = as.numeric(proteome_expression_data[cor_value1$from[idx], ]),
...
#     labs(
#       x = paste("Exposome (Biological): ", cor_value1$proteome_id[idx], sep = ""),
#       y = paste("environment: " , cor_value1$environment_id[idx]),
#       sep = ""
#     ) +
#     theme_bw() +
#     theme(
...
setwd(r4projects::get_project_wd())
setwd("3_data_analysis/data_20200511/proteome_environment")

cor_value1$from %>% unique()

###correlation network for proteome and environment
...
###network for all the proteome and environment
edge_data <-
  cor_value1 %>%
...
node_data <-
  ...
  dplyr::mutate(class1 = case_when(
    stringr::str_detect(class, "from") ~ "Proteome",
    TRUE ~ "Enviroment"
  )) %>%
...
ggsave(
  plot1,
  filename = "proteome_environment_correlation_network.pdf",
  width = 8.5,
  height = 7,
  bg = "transparent"
)
...
ggsave(
  plot1,
  filename = "proteome_environment_correlation_network.png",
  width = 8.5,
  height = 7,
  bg = "transparent"
)
