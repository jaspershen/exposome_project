...
metabolome_annotation_info <-
  readr::read_csv("3_data_analysis/data_20200511/metabolome/annotation_table.csv")

metabolome_annotation_info %>% dim()

variable_info <-
  variable_info %>%
  left_join(metabolome_annotation_info, by = c("peak_name" = "name"))
...
setwd("3_data_analysis/data_20200511/environment_metabolome")

dim(environment_variable_info)

dim(metabolome_variable_info)
...
diff <-
  lapply(metabolome_sample_info$CollectionDate %>% as.Date(), function(x) {
    x <- x - as.Date(environment_sample_info$date.start)
    as.Date(environment_sample_info$date.start) [which(x >= 0 &
                                                        x <= 3)]
  })

names(diff) <- metabolome_sample_info$CollectionDate %>% as.character()
...
plot <-
  ...
  dplyr::mutate(class = factor(class, levels = c("met", "environment"))) %>%
  ...
  # scale_y_continuous(breaks = c("met", "environment"), labels = c("Met", "Environment")) +
  scale_y_discrete(
    breaks = c("met", "environment"),
    labels = c("Metabolome", "Environment")
  ) +
  labs(x = "", y = "") +
...
ggsave(plot,
       filename = "environment_metabolome_match.pdf",
       width = 10,
       height = 7)

ggsave(plot,
       filename = "environment_metabolome_match.png",
       width = 10,
       height = 7)

ggsave(plot,
       filename = "environment_metabolome_match2.png",
       width = 14,
       height = 7)
...
metabolome_expression_data <-
  metabolome_expression_data %>%
  dplyr::select(one_of(metabolome_sample_info$sample_id))
