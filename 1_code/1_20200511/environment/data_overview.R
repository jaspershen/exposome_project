...
setwd("3_data_analysis/data_20200511/environment/")
...
###coorplot of all the environment

library(corrplot)
library(corrr)
col3 <- colorRampPalette(c("red", "white", "blue")) 
temp_data = corrr::correlate(t(expression_data), method = "spearman", diagonal = 1) %>%
  tibble::column_to_rownames(var = "term")

colnames(temp_data) = 
  rownames(temp_data) =
  variable_info$true_name
